...
class Request(Resource):
    ...
    # get all column details of corresponding requestId
        ...
        confirmed = 0
        query = "SELECT confirmed FROM requests WHERE requestId=?"
        result = cursor.execute(query,(requestId,))
        for row in result:
            confirmed = row[0]

        confirmed_with_hotel = ""
        query = "SELECT confirmed_with_hotel FROM requests WHERE requestId=?"
        result = cursor.execute(query,(requestId,))
        for row in result:
            confirmed_with_hotel = row[0]

        budget = 0
        query = "SELECT budget FROM requests WHERE requestId=?"
        result = cursor.execute(query,(requestId,))
        for row in result:
            budget = row[0]

        food = ""
        query = "SELECT food FROM requests WHERE requestId=?"
        result = cursor.execute(query,(requestId,))
        for row in result:
            food = row[0]

        snacks = 0
        query = "SELECT snacks FROM requests WHERE requestId=?"
        result = cursor.execute(query,(requestId,))
        for row in result:
            snacks = row[0]
        ...
        return {
                ...
                "rooms": rooms,
                "confirmed": confirmed,
                "confirmed_with_hotel": confirmed_with_hotel,
                "budget": budget,
                "food": food,
                "snacks": snacks
            }
    ...
    def confirmBid(self, requestId, hotel_name, flag):
        if requestId == 0:
            return "", 404

        connection = sql.connect('data.db')
        cursor = connection.cursor()
        query = "UPDATE requests SET confirmed=?, confirmed_with_hotel=? WHERE requestId=?"
        cursor.execute(query, (flag, hotel_name, requestId, ))
        connection.commit()
        connection.close()
...
class RequestList(Resource):
    def get(self):
        ...
        requests = []
        for row in result:
            requests.append({"id": row[0], "hall": row[1], "localityId": row[2], "date": row[3], "people": row[4], "food": row[5], "snacks": row[6], "rooms": row[7], "budget": row[8], "confirmed": row[9]})


        connection.close()
        return {"requests":requests}, 200

    def post(self, userid_from_website):
        connection = sql.connect('data.db')
        cursor = connection.cursor()
...
# add request to user
        ...
        userId = userid_from_website
        if (userId == 0):
            abort(404, message="user has not logged in!")

        from requestsUsers import RequestsUsers
        obj = RequestsUsers()
        status = obj.add_resquestsUsers(new_requestId,userId,connection,cursor)
        del obj 
